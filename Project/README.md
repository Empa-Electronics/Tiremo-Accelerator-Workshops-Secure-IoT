...
- Project builds with **eMStudio32 (Eclipse + GCC 10.3.1)** — see [SetUp.md](../SetUp.md) for installation.
- Import via **ABOV → Open eMStudio32 Project** (do not use a generic Eclipse).
- Eclipse project files (`.project`, `.cproject`, `.settings/`, `makefile`, `subdir.mk`) use **relative paths** so the repo is portable after clone.
- `subdir.mk` files are auto-generated by Eclipse; commit them only if they contain relative paths (no `C:/Users/...`).
- Add new `.c` files via Eclipse Project Explorer.
- After clone or import: **Project → Clean… → Clean**, then **Build**.
- Do **not** commit build outputs (`.o`, `.d`, `.elf`, `.hex`) — see root `.gitignore`.

### Publishing to GitHub (maintainers)

So participants can **clone and build** without path or toolchain surprises:

| Commit | Do not commit |
|--------|----------------|
| Source under `Example/Source/TmplUserApp/` | `A34G43x/**/*.o`, `*.d`, `*.elf`, `*.hex` (build outputs) |
| Eclipse project: `.project`, `.cproject`, `.settings/` | `Workspace/.metadata/` (local Eclipse workspace) |
| `makefile`, `subdir.mk`, `sources.mk` with **relative** paths | Absolute paths like `C:/Users/...` in any `.mk` file |
| Pre-built workshop HEX in repo `Binary/` | PEM/KEY credential files (workshop uses `.inc` only) |
| `SetUp.md`, `RunningCode.md`, docs | Machine-specific IDE preferences outside the project folder |

Before pushing: search the repo for `C:/Users/` and `--no-warn-rwx-segment`. Remove any hits. Participants still need a **full eMStudio32 install** (including xpack build tools) — that cannot be bundled in Git.
...
|---------|--------------|
| `Program "make" not found in PATH` | eMStudio32 not fully installed, or IDE not started from ABOV shortcut; xpack `make.exe` missing — see [SetUp.md](../SetUp.md) |
| `ld.exe: unrecognized option '--no-warn-rwx-segment'` | Toolchain is GCC 10.3 (bundled with eMStudio32); pull latest repo or remove that linker flag |
